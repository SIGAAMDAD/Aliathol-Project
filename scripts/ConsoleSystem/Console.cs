...
public partial class Console : Control {
	...
	*/
	...
	private void InitializeCommands() {
		...
		AddCommand( "cmdlist", new Callable( this, MethodName.CommandsList ), [], 0, "Lists all commands and their descriptions." );
	...
	*/
	...
	private void Help() {
		RichLabel.AppendText(
			...
			[color=light_green]cmdlist[/color]: Shows a detailed list of all the currently registered commands
	...
	*/
	...
	public override void _EnterTree() {
		...
		LineEdit = new LineEdit() {
			Name = nameof( LineEdit ),
			AnchorTop = 0.5f,
			AnchorRight = 1.0f,
			AnchorBottom = 0.5f,
			PlaceholderText = "Enter \"help\" for instructions",
		};
		Control.AddChild( LineEdit );
		GameEventBus.ConnectSignal( LineEdit, LineEdit.SignalName.TextSubmitted, this, Callable.From<string>( OnTextEntered ) );
		GameEventBus.ConnectSignal( LineEdit, LineEdit.SignalName.TextChanged, this, Callable.From<string>( OnLineEditTextChanged ) );

		ProcessMode = ProcessModeEnum.Always;

		// Load history
		using FileAccess file = FileAccess.Open( "user://console_history.txt", FileAccess.ModeFlags.Read );
		if ( file != null ) {
			while ( !file.EofReached() ) {
